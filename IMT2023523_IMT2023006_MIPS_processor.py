from IMT2023523_IMT2023006_MIPS_components import *
...
class Processor:
    ...
    def mem_access(self, data=None):
        """Class method to read and write from memory during the mem access stage"""
        if self.memRead:
            data = self.mem.read(self.alu.ALU_result)
            return data
        # ALU result is used for the address to be written to, and data corresponds to the data that is on the bus going into write data port
        if data and self.memWrite == 1:
            self.mem.write(self.alu.ALU_result, data)
